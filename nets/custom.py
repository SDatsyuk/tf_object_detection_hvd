...
_CONV_DEFS = [
    ...
    Conv(kernel=[2, 2], stride=1, depth=32),
    Conv(kernel=[2, 2], stride=2, depth=32),
    Conv(kernel=[2, 2], stride=1, depth=32),
    Conv(kernel=[2, 2], stride=2, depth=32),
    Conv(kernel=[2, 2], stride=1, depth=32),
    Conv(kernel=[2, 2], stride=2, depth=64),
    Conv(kernel=[2, 2], stride=1, depth=64),
    Conv(kernel=[2, 2], stride=1, depth=64),
    Conv(kernel=[2, 2], stride=1, depth=64)
]
  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs], reuse=reuse) as scope:
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        ...
        logits = slim.dense(logits, eos_token=128)
        logits = tf.layers.batch_normalization(logits, training=is_train)
        logits = nn.relu(logits)
        logits = slim.dropout(logits, keep_prob=0.5)
        logits = slim.dense(logits, num_classes)
